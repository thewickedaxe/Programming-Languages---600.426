...
type number = Integer of int | Floating of float ;;

type token = Number of number | Plus | Minus | Mul | Div ;;

let smart_min a b =
    match (a, b) with
    | (Floating(c), Floating(d)) -> Floating(c -. d)
    | (Integer(c), Integer(d)) -> Integer(c-d)
    | (Floating(c), Integer(d)) -> Floating(c -. float_of_int(d))
    | (Integer(c), Floating(d)) -> Floating(float_of_int(c) -. d)
;;
...
let smart_div a b =
    match (a, b) with
    | (_, Integer(0)) -> invalid_arg "division by zero"
    | (_, Floating(0.)) -> invalid_arg "division by zero"
    | (Floating(c), Floating(d)) -> Floating(c /. d)
    | (Integer(c), Integer(d)) -> Integer(c/d)
    | (Floating(c), Integer(d)) -> Floating(c /. float_of_int(d))
    | (Integer(c), Floating(d)) -> Floating(float_of_int(c) /. d)
